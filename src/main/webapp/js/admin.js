...
function course_avg() {
    ...
    var url = "AdminDao?action=course_avg";
...
function fail_rate() {
    ...
    var url = "AdminDao?action=fail_rate";
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//显示查询成绩排名信息
...
function course_ranking() {
    ...
    var url = "AdminDao?action=course_ranking&cno=" + Cno;
...
function query_one_sc() {
    ...
    var user =  document.getElementById("query_one").getElementsByTagName("input");
    var sno = user[0].value.toString();
    var sname = user[1].value.toString();
    xmlhttp.open("GET", "AdminDao?action=query_one_sc&sno=" + sno+"&sname="+sname, true);
    xmlhttp.send();
}
...
function download_file() {
    ...
    form.method = 'GET';
    form.action = '/AdminDao'; // 这里要对应后端Servlet的映射路径

    // 添加参数到表单，这里假设添加一个名为param，值为value的参数
    var paramInput = document.createElement('input');
    paramInput.type = 'hidden'; // 设置为隐藏类型，这样用户看不到这个输入框
    paramInput.name = 'action';
    paramInput.value = 'download_file';
...
function query_one() {
    var result = document.getElementById("result");
    var show =  "<div id='query_one'  class='d_form'>"
        + "<h3>请输入查询信息</h3>"
        + "<input type='text' autofocus='autofocus' name='sno' value placeholder='学号' required>"
        + "<input type='text' name='sname' value placeholder='姓名' required>"
        + "<input id='submit' onclick=query_one_sc() type='button' name='submit' value='查询'>"
        + "</div>";
    result.innerHTML = show;
}
...
function delete_user() {
    ...
    var username = user[0].value.toString();
    var url = "AdminDao?action=delete_user&username=" + username;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//删除院系
function delete_department() {
    ...
    var url = "AdminDao?action=delete_department&dno=" + dno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//删除班级
function delete_class() {
    ...
    var url = "AdminDao?action=delete_class&clno=" + clno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//删除学生
function delete_student() {
    ...
    var url = "AdminDao?action=delete_student&sno=" + sno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//删除课程
function delete_course() {
    ...
    var url = "AdminDao?action=delete_course&cno=" + cno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//删除成绩
function delete_sc() {
    ...
    var url = "AdminDao?action=delete_sc&sno=" + sno + "&cno="+cno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
/*-----------------------------------修改---------------------------------*/
...
function alter_user() {
    ...
    var url = "AdminDao?action=alter_user&username=" + username + "&after_username=" + after_username + "&after_password=" + after_password + "&after_level=" + after_level;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改院系
function alter_department() {
    ...
    var url = "AdminDao?action=alter_department&dno=" + dno + "&after_dno=" + after_dno + "&after_dname" + after_dname;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改班级
function alter_class() {
    ...
    var url = "AdminDao?action=alter_class&clno=" + clno + "&after_clno=" + after_clno + "&after_clname=" + after_clname + "&after_dno=" + after_dno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改学生
function alter_student() {
    ...
    var url = "AdminDao?action=alter_student&sno=" + sno + "&after_sno=" + after_sno + "&after_sname=" + after_sname + "&after_ssex=" + after_ssex+"&after_sage="+after_sage+"&after_clno="+after_clno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改课程
function alter_course() {
    ...
    var url = "AdminDao?action=alter_course&cno=" + cno + "&after_cno=" + after_cno + "&after_cname=" + after_cname + "&after_cteacher=" + after_cteacher+"&after_ccredit="+after_ccredit;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改成绩
function alter_sc() {
    ...
    var url = "AdminDao?action=alter_sc&sno=" + sno + "&cno=" + cno + "&after_grade=" + after_grade;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}